...
function makeAutoSignal(ctx, dur = 2.5) {
...
export default function CompressorTrain({ exercise, onComplete }) {
  ...
  const playWith = (useTarget) => {
    ...
    if (sigBufRef.current) {
      audioBuf = ctx.createBuffer(1, sigBufRef.current.length, ctx.sampleRate);
      audioBuf.getChannelData(0).set(sigBufRef.current);
    } else {
      audioBuf = makeAutoSignal(ctx);
    }
  ...
  return (
    <div>
      ...
      <p style={{ color: C.muted, margin: '0 0 18px', fontSize: 13 }}>התאם את הקומפרסור לצליל ה-Reference (A)</p>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 220px', gap: 18 }}>
          ...
          {(phase === 'playing' || phase === 'answered') && (
            <>
              <div style={{ display: 'flex', alignItems: 'center', gap: 12, marginBottom: 20 }}>
                <button onClick={toggleAB} disabled={phase === 'answered'}
                  style={{ padding: '9px 28px', background: mode === 'A' ? '#4488ff' : C.y, color: '#000', border: 'none', borderRadius: 7, fontWeight: 900, fontSize: 14, cursor: 'pointer', opacity: phase === 'answered' ? 0.5 : 1 }}>
        ...
        <div style={{ background: C.card, padding: 20, borderRadius: 12, border: '1px solid ' + C.borderLit }}>
          ...
          <div style={{ marginTop: 16, fontSize: 11, color: C.muted, lineHeight: 1.7 }}>
            <div>💡 <b style={{ color: C.text }}>Threshold</b> — רמה בה הקומפרסור מתחיל לעבוד</div>
